...
- **Q1** ~~네이버 검색·카카오 로컬·쿠팡 파트너스 각각의 저장 허용 범위·캐시 TTL·출처표기·호출 제한 확정.~~ → **해소(ADR-0003)**: 세 경로 모두 비영속·실시간 표시·출처표기. 쿠팡 대가성 문구 의무(INV-10).
- **Q2** 영양 DB ↔ 품목제조보고 로스터 커버리지 갭 실측.
- **Q3** ~~라면/라멘 도메인 필터의 식품유형코드·업태코드 매핑 확정.~~ → **해소(ADR-0004)**: 전용 코드 없음. 제품=면류(유탕면·건면)+키워드+보정, 음식점=일식·분식 업태+상호 키워드+보정.
