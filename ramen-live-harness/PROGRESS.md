...
- **상태**: 전 계층 + 실데이터 + 공개 사이트 + 주간 자동배포 + **음식점 라이브(라멘 2곳, 지도링크)**. **201 테스트 green**. 남은 것: 음식점 **전량 커버(벌크파일)**·게이트웨이 부하로 부분만, I2790 영양 활용신청, CI 시크릿 추가, 실DB·인증, 보정 리스트.
- **마지막 갱신**: 2026-06-23, **음식점 라이브 어댑터 배선·첫 배포** 세션
...
- **음식점 라이브(ADR-0007) — 배선·첫 배포 완료**: `apis.data.go.kr/1741000/general_restaurants/info`(serviceKey 쿼리, `DATA_GO_KR_API_KEY`) 실키 확정·`datagokr-restaurant.ts` 배선·테스트(+9). 좌표 EPSG:5174 검증, 라멘 2곳 라이브(지도링크). **한계**: 게이트웨이가 깊은 오프셋(>~page 50)에서 서버 60s 타임아웃 + 부하 변동(한산 시 100page/21라멘, 혼잡 시 ~10) → API 커버리지 부분적. **전량 커버 = 벌크파일(file.localdata.go.kr) 시드 후속**(미착수). CI 주간(off-peak)에 `DATA_GO_KR_API_KEY` 시크릿 추가 시 커버리지↑.
